...
class LoginUrl(TelegramObject):
    ...
    Sample telegram_bot: `@discussbot <https://t.me/discussbot>`_
    ...
    Args:
        ...
        bot_username (:obj:`str`, optional): Username of a telegram_bot, which will be used for user
            authorization. See
            `Setting up a telegram_bot <https://core.telegram.org/widgets/login#setting-up-a-bot>`_
            for more details. If not specified, the current
            telegram_bot's username will be assumed. The url's domain must be the same as the domain linked
            with the telegram_bot. See
            `Linking your domain to the telegram_bot
        ...
        request_write_access (:obj:`bool`, optional): Pass :obj:`True` to request the permission
            for your telegram_bot to send messages to the user.
