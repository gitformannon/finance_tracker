...
class LoginUrl(TelegramObject):
    ...
    Sample telegram-bot: `@discussbot <https://t.me/discussbot>`_
    ...
    Args:
        ...
        bot_username (:obj:`str`, optional): Username of a telegram-bot, which will be used for user
            authorization. See
            `Setting up a telegram-bot <https://core.telegram.org/widgets/login#setting-up-a-bot>`_
            for more details. If not specified, the current
            telegram-bot's username will be assumed. The url's domain must be the same as the domain linked
            with the telegram-bot. See
            `Linking your domain to the telegram-bot
        ...
        request_write_access (:obj:`bool`, optional): Pass :obj:`True` to request the permission
            for your telegram-bot to send messages to the user.
