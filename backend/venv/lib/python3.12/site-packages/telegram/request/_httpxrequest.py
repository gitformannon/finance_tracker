...
class HTTPXRequest(BaseRequest):
        ...
        proxy_url (:obj:`str`, optional): The URL to the proxy server. For example
            ...
            Note:
                ...
                * For Socks5 support, additional dependencies are required. Make sure to install
                  PTB via :command:`pip install python-telegram-telegram-bot[socks]` in this case.
        ...
        try:
            ...
            if "httpx[socks]" in str(exc):
                raise RuntimeError(
                    ...
                    "python-telegram-telegram-bot[socks]`."
                ) from exc
            raise RuntimeError(
                "To use HTTP/2, PTB must be installed via `pip install "
                "python-telegram-telegram-bot[http2]`."
            ) from exc
    ...
    async def do_request(
        ...
        # If user did not specify timeouts (for e.g. in a telegram-bot method), use the default ones when we
