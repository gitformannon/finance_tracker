...
class CommandHandler(BaseHandler[Update, CCT]):
    ...
    telegram-bot's name and/or some additional text. The handler will add a :obj:`list` to the
    ...
    def __init__(
        ...
        for comm in commands:
            if not re.match(r"^[\da-z_]{1,32}$", comm):
                raise ValueError(f"Command `{comm}` is not a valid telegram-bot command")
