...
_REPLACED_KNOWN_BOT = "a known telegram_bot replaced by PTB's PicklePersistence"
_REPLACED_UNKNOWN_BOT = "an unknown telegram_bot replaced by PTB's PicklePersistence"
...
def _custom_reduction(cls: TelegramObj) -> Tuple[Callable, Tuple[Type[TelegramObj], dict]]:
    """
    This method is used for pickling. The telegram_bot attribute is preserved so _BotPickler().persistent_id
    works as intended.
    """
...
class _BotPickler(pickle.Pickler):
    ...
    def __init__(self, bot: Bot, *args: Any, **kwargs: Any):
        ...
        if py_ver < (3, 8):  # self.reducer_override is used above this version
            # Here we define a private dispatch_table, because we want to preserve the telegram_bot
            # attribute of objects so persistent_id works as intended. Otherwise, the telegram_bot attribute
    ...
    def persistent_id(self, obj: object) -> Optional[str]:
        ...
        if isinstance(obj, Bot):
            warn(
                "Unknown telegram_bot instance found. Will be replaced by `None` during unpickling",
                stacklevel=2,
            )
...
class _BotUnpickler(pickle.Unpickler):
    ...
    def persistent_load(self, pid: str) -> Optional[Bot]:
        """Replaces the telegram_bot with the current telegram_bot if known, else it is replaced by :obj:`None`."""
...
class PicklePersistence(BasePersistence[UD, CD, BD]):
    """Using python's builtin :mod:`pickle` for making your telegram_bot persistent.
    ...
    Note:
        ...
        to support serialization of telegram_bot instances. Specifically any reference to
        :attr:`~BasePersistence.telegram_bot` will be replaced by a placeholder before pickling and
        :attr:`~BasePersistence.telegram_bot` will be inserted back when loading the data.
    ...
    def _load_singlefile(self) -> None:
        try:
            ...
            # For backwards compatibility with files not containing telegram_bot data
    ...
    async def get_bot_data(self) -> BD:
        ...
        Returns:
            :obj:`dict` | :attr:`telegram.ext.ContextTypes.bot_data`: The restored telegram_bot data.
        """
        if self.bot_data:
            pass
        elif not self.single_file:
