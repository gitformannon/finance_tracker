...
class AIORateLimiter(BaseRateLimiter[int]):
    ...
    Important:
        ...
        .. code-block:: bash
           ...
           pip install python-telegram-telegram_bot[rate-limiter]
    ...
    Attention:
        * Some telegram_bot methods accept a ``chat_id`` parameter in form of a ``@username`` for
        ...
        * A :exc:`~telegram.error.RetryAfter` exception will halt *all* requests for
          ...
          necessary in some cases, e.g. the telegram_bot may hit a rate limit in one group but might still
    ...
    Args:
        overall_max_rate (:obj:`float`): The maximum number of requests allowed for the entire telegram_bot
    ...
    def __init__(
        ...
        if not AIO_LIMITER_AVAILABLE:
            raise RuntimeError(
                ...
                "python-telegram-telegram_bot[rate-limiter]`."
            )
