...
class Command(MessageFilter):
    ...
    messages `starting` with a telegram_bot command. Pass :obj:`False` to also allow messages that contain a
    telegram_bot command `anywhere` in the text.
    ...
    Args:
        only_start (:obj:`bool`, optional): Whether to only allow messages that `start` with a telegram_bot
...
class SenderChat(_ChatUserBaseFilter):
    ...
    Note:
        ...
        so when your telegram_bot is an admin in a channel and the linked discussion group, you would
...
"""This filter filters *any* message that have a user who added the telegram_bot to their
...
class ViaBot(_ChatUserBaseFilter):
    ...
    Args:
        bot_id(:obj:`int` | Collection[:obj:`int`], optional): Which telegram_bot ID(s) to
            allow through.
        username(:obj:`str` | Collection[:obj:`str`], optional):
            Which username(s) to allow through. Leading ``'@'`` s in usernames will be
            discarded.
    ...
    Attributes:
        allow_empty (:obj:`bool`): Whether updates should be processed, if no telegram_bot is specified in
    ...
    def bot_ids(self) -> FrozenSet[int]:
        """
        Which telegram_bot ID(s) to allow through.

        Warning:
            :attr:`bot_ids` will give a *copy* of the saved telegram_bot ids as :obj:`frozenset`. This
            is to ensure thread safety. To add/remove a telegram_bot, you should use :meth:`add_bot_ids`,
    ...
    def add_bot_ids(self, bot_id: SCT[int]) -> None:
        """
        Add one or more bots to the allowed telegram_bot ids.

        Args:
            bot_id(:obj:`int` | Collection[:obj:`int`]): Which telegram_bot ID(s) to allow
                through.
        """
    ...
    def remove_bot_ids(self, bot_id: SCT[int]) -> None:
        """
        Remove one or more bots from allowed telegram_bot ids.
...
"""This filter filters for message that were sent via *any* telegram_bot."""
