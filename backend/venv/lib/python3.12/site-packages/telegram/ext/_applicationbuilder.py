...
class ApplicationBuilder(Generic[BT, CCT, UD, CD, BD, JQ]):
    ...
    Note:
        * Some arguments are mutually exclusive. E.g. after calling :meth:`token`, you can't set
          a custom telegram_bot with :meth:`telegram_bot` and vice versa.
        * Unless a custom :class:`telegram.Bot` instance is set via :meth:`telegram_bot`, :meth:`build` will
          use :class:`telegram.ext.ExtBot` for the telegram_bot.
    ...
    def _build_ext_bot(self) -> ExtBot:
        if isinstance(self._token, DefaultValue):
            raise RuntimeError("No telegram_bot token was set.")
    ...
    def _bot_check(self, name: str) -> None:
        if self._bot is not DEFAULT_NONE:
            raise RuntimeError(_TWO_ARGS_REQ.format(name, "telegram_bot instance"))
    ...
    def build(
        ...
        if isinstance(self._updater, DefaultValue) or self._updater is None:
            if isinstance(self._bot, DefaultValue):  # and didn't set a telegram_bot
                bot: Bot = self._build_ext_bot()  # build a telegram_bot
            else:
                bot = self._bot
        ...
        if persistence is not None:
            ...
            # but self.telegram_bot is not an instance of ExtBot - so no need to check that later on
    ...
    def token(self: BuilderType, token: str) -> BuilderType:
        """Sets the token for :attr:`telegram.ext.Application.telegram_bot`.
    ...
    def base_url(self: BuilderType, base_url: str) -> BuilderType:
        """Sets the base URL for :attr:`telegram.ext.Application.telegram_bot`. If not called,
    ...
    def base_file_url(self: BuilderType, base_file_url: str) -> BuilderType:
        """Sets the base file URL for :attr:`telegram.ext.Application.telegram_bot`. If not
    ...
    def _request_param_check(self, name: str, get_updates: bool) -> None:
        ...
        if self._bot is not DEFAULT_NONE:  # disallow request args if telegram_bot is set (has Request)
            raise RuntimeError(
                _TWO_ARGS_REQ.format(
                    f"get_updates_{name}" if get_updates else name, "telegram_bot instance"
                )
            )

        if self._updater not in (DEFAULT_NONE, None):  # disallow request args for updater(has telegram_bot)
            raise RuntimeError(
    ...
    def request(self: BuilderType, request: BaseRequest) -> BuilderType:
        ...
        :paramref:`telegram.Bot.request` parameter of :attr:`telegram.ext.Application.telegram_bot`.
    ...
    def http_version(self: BuilderType, http_version: str) -> BuilderType:
        ...
        Note:
            ...
            python-telegram-telegram_bot/issues/3556>`_ for a discussion.
            ...
            .. code-block:: bash
               ...
               pip install python-telegram-telegram_bot[http2]
    ...
    def get_updates_request(self: BuilderType, get_updates_request: BaseRequest) -> BuilderType:
        ...
        :attr:`telegram.ext.Application.telegram_bot`.
    ...
    def private_key(
        ...
        for :attr:`telegram.ext.Application.telegram_bot`.
    ...
    def defaults(self: BuilderType, defaults: "Defaults") -> BuilderType:
        ...
        .. seealso:: :wiki:`Adding Defaults to Your Bot <Adding-defaults-to-your-telegram_bot>`
    ...
    def arbitrary_callback_data(
        ...
        """Specifies whether :attr:`telegram.ext.Application.telegram_bot` should allow arbitrary objects as
        ...
        Important:
            ...
            .. code-block:: bash
               ...
               pip install python-telegram-telegram_bot[callback-data]
    ...
    def local_mode(self: BuilderType, local_mode: bool) -> BuilderType:
        ...
        Args:
            local_mode (:obj:`bool`): Whether the telegram_bot should run in local mode.
    ...
    def bot(
        ...
        :attr:`telegram.ext.Application.telegram_bot`. Instances of subclasses like
        :class:`telegram.ext.ExtBot` are also valid.

        Args:
            bot (:class:`telegram.Bot`): The telegram_bot.

        Returns:
            :class:`ApplicationBuilder`: The same builder with the updated argument.
        """
        self._updater_check("telegram_bot")
        for attr, error in _BOT_CHECKS:
            if not isinstance(getattr(self, f"_{attr}"), DefaultValue):
                raise RuntimeError(_TWO_ARGS_REQ.format("telegram_bot", error))
        self._bot = bot
    ...
    def concurrent_updates(self: BuilderType, concurrent_updates: Union[bool, int]) -> BuilderType:
        ...
        Warning:
            ...
            that your telegram_bot does not (explicitly or implicitly) rely on updates being processed
            sequentially.
    ...
    def persistence(
        ...
        .. seealso:: :wiki:`Making Your Bot Persistent <Making-your-telegram_bot-persistent>`
    ...
    def updater(self: BuilderType, updater: Optional[Updater]) -> BuilderType:
        ...
        :attr:`telegram.ext.Application.updater`. The :attr:`telegram.ext.Updater.telegram_bot` and
        :attr:`telegram.ext.Updater.update_queue` will be used for
        :attr:`telegram.ext.Application.telegram_bot` and :attr:`telegram.ext.Application.update_queue`,
        respectively.
        ...
        for attr, error in (
            (self._bot, "telegram_bot instance"),
            (self._update_queue, "update_queue"),
        ):
        ...
        Example:
            .. code::
                ...
                async def post_init(application: Application) -> None:
                    await application.telegram_bot.set_my_commands([('start', 'Starts the telegram_bot')])
    ...
    def post_stop(
        ...
        Tip:
            ...
            sending message to a chat before shutting down the telegram_bot

        Example:
            .. code::

                async def post_stop(application: Application) -> None:
                    await application.telegram_bot.send_message(123456, "Shutting down...")
