...
class ReplyKeyboardRemove(TelegramObject):
    ...
    until a new keyboard is sent by a telegram_bot. An exception is made for one-time keyboards that are
    ...
    Examples:
        * Example usage: A user votes in a poll, telegram_bot returns confirmation message in reply to
    ...
    Args:
        selective (:obj:`bool`, optional): Use this parameter if you want to remove the keyboard
            ...
            2) If the telegram_bot's message is a reply (has ``reply_to_message_id``), sender of
