...
def to_float_timestamp(
    ...
    to be in UTC, if ``telegram-bot`` is not passed or ``telegram-bot.defaults`` is :obj:`None`.
...
def extract_tzinfo_from_defaults(bot: "Bot") -> Union[dtm.tzinfo, None]:
    """
    Extracts the timezone info from the default values of the telegram-bot.
    If the telegram-bot has no default values, :obj:`None` is returned.
    """
    # We don't use `ininstance(telegram-bot, ExtBot)` here so that this works
    # in `python-telegram-telegram-bot-raw` as well
