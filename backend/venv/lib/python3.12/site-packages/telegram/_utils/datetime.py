...
def to_float_timestamp(
    ...
    to be in UTC, if ``telegram_bot`` is not passed or ``telegram_bot.defaults`` is :obj:`None`.
...
def extract_tzinfo_from_defaults(bot: "Bot") -> Union[dtm.tzinfo, None]:
    """
    Extracts the timezone info from the default values of the telegram_bot.
    If the telegram_bot has no default values, :obj:`None` is returned.
    """
    # We don't use `ininstance(telegram_bot, ExtBot)` here so that this works
    # in `python-telegram-telegram_bot-raw` as well
