...
class InlineQueryResultsButton(TelegramObject):
    ...
    Args:
        ...
        start_parameter (:obj:`str`, optional):  Deep-linking parameter for the
            ...
            Example:
                An inline telegram_bot that sends YouTube videos can ask the user to connect the telegram_bot to
                ...
                any. The user presses the button, switches to a private chat with the telegram_bot and, in
                doing so, passes a start parameter that instructs the telegram_bot to return an OAuth link.
                Once done, the telegram_bot can offer a switch_inline button so that the user can easily
                return to the chat where they wanted to use the telegram_bot's inline capabilities.
